...
def sigmoid_entropy(x):
    p = torch.sigmoid(x)
    return -(p * torch.log(p + 1e-6) + (1 - p) * torch.log(1 - p + 1e-6)).sum(1)
...
def clip_classifier(classnames, template, clip_model):
    with torch.no_grad():
        ...
        for classname in classnames:
            ...
            class_embeddings = clip_model.encode_text(texts).half()
...
def get_clip_logits(images, clip_model, clip_weights):
    with torch.no_grad():
        ...
        if hasattr(clip_model, "get_image_features"):
            ...
            image_features = clip_model.encode_image(images).half()
...
def get_clip_logits_retrieval(images, clip_model, clip_weights, clip_retrieval_client):
    with torch.no_grad():
        if isinstance(images, list):
            images = torch.cat(images, dim=0).cuda()
        else:
        ...
        # retrieval
        image_features = image_features.cpu().detach().numpy().astype("float32")[0]
        image_features_retrieved = clip_retrieval_client.query(embedding_input=image_features.tolist())
        image_features_retrieved = torch.tensor(image_features_retrieved).cuda()
        image_features_retrieved /= image_features_retrieved.norm(dim=-1, keepdim=True)
        print(image_features_retrieved.shape)
        exit()
...
def get_clip_logits_dn(images, clip_model, clip_weights, image_feature_mean, clip_logits_mean):
    with torch.no_grad():
        if isinstance(images, list):
            images = torch.cat(images, dim=0).cuda()
        else:
        ...
        image_features = image_features - image_feature_mean * 0.25
        clip_wieghts = clip_weights - clip_logits_mean * 0.25

        image_features /= image_features.norm(dim=-1, keepdim=True)
        clip_wieghts /= clip_wieghts.norm(dim=0, keepdim=True)
